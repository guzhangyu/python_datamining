...
class ArimaModel:
    ...
    def __init__(self,data,white_noise_p):
        self.data=data
        ...
        for p in range(pmax + 1):
            ...
            for q in range(qmax + 1):
                try:  # 存在部分报错，所以用try来跳过报错。
                    tmp.append(ARIMA(self.data, (p,1 if self.diff>0 else 0, q)).fit(disp=5).bic)
    ...
    def gen_model(self):
        self.model = ARIMA(self.data, (self.p, 1 if self.diff>0 else 0, self.q)).fit()  # 建立并训练模型
        if self.diff>0:
    ...
    def white_noise_test(self,data,s):
        ...
        if h > pv.shape[0]*self.white_noise_p:
