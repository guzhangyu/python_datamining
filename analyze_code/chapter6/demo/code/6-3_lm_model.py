...
net.fit(train[:,:3], train[:,3], nb_epoch=1000, batch_size=16) #训练模型，循环1000次
net.save_weights(netfile) #保存模型

predict_result = net.predict_classes(train[:,:3],batch_size=16).reshape(len(train)) #预测结果变形
...
predict_result = net.predict(test[:,:3],batch_size=16).reshape(len(test))
